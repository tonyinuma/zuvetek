...
use Modules\Account\Exports\ReportAccountingConcarGlosaExport;
...
class AccountController extends Controller
{
    ...
    public function download(Request $request)
    {
        ...
        switch ($type) {
            case 'concar_glosa':
                $data = [
                    'records' => $this->getStructureConcarGlosa($this->getAllDocuments($d_start, $d_end)),
                ];

                $report = (new ReportAccountingConcarGlosaExport)
                            ->data($data)
    ...
    private function getStructureConcarGlosa($documents)
    {
        $company_account = CompanyAccount::first();
        $rows = [];
            ...
            foreach ($row->items as $item) {
                ...
                if(in_array($row->document_type_id, ['07']))
                {
                    ...
                    $rows[] = [
                        ...
                        // 'col_W' => $detail,
                        'col_X' => '',
                        'col_Y' => $document_type_id.'-'.$row->number_full,
